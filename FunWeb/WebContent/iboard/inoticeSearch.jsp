...
//글 삭제시 DB번호(num)가 사라짐 -> 게시판내 자체 번호 구하기
int num = 0; // 초기값 선언
//전체 글에 페이지 와 갯수에 따른 번호를 구함
//전체 글갯수 count        페이지 currentPage -0               시작번호
//   30                   - 1-1 => 0*10 = 0    =>      30
//   30                   - 2-1 => 1*10 = 10   =>      20
//   30                   - 3-1 => 2*10 = 20   =>      10   
...
<tr><th class="tno">No.</th>
    ...
    if(count != 0) {
    	...
    	<%BoardBean3 bb1 = bdao.getBoard(num); %>
    <tr onclick="location.href='icontent.jsp?num=<%=bb.getNum()%>&pageNum=<%=pageNum%>'">
    <td><%=num--%>
    <img src="../upload/<%=bb.getFile()%>" width="150" height="150"></td>
...
%>    
<div id="table_search">
<form action="inoticeSearch.jsp" method="get">
...
if(startPage > pageBlock) { // pageSize 이상이 되어야 [이전] 이 보임
	%><a href="inoticeSearch.jsp?pageNum=<%=startPage-pageBlock %>&search=<%=search%>">Prev</a><%
}

for(int i = startPage; i <= endPage; i++) {
	%><a href="inoticeSearch.jsp?pageNum=<%=i %>&search=<%=search%>"><%=i %></a> <%
}

// [다음]
if(endPage < pageCount) {
	%><a href="inoticeSearch.jsp?pageNum=<%=startPage+pageBlock %>&search=<%=search%>">Next</a><%
}
%>
<!-- 		1 2 3 .... 10 [다음] -->
<!-- [이전]	11 12 13 .... 20	 -->
