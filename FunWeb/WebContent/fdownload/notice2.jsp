...
<tr><th class="tno">No.</th>
    ...
    if(count != 0) {
    	...
    	%>
    <tr onclick="location.href='content2.jsp?num=<%=bb.getNum()%>&pageNum=<%=pageNum%>'"><td><%=num--%></td>
...
if(startPage > pageBlock) { // pageSize 이상이 되어야 [이전] 이 보임
	%><a href="notice2.jsp?pageNum=<%=startPage-pageBlock %>">Prev</a><%
}

for(int i = startPage; i <= endPage; i++) {
	%><a href="notice2.jsp?pageNum=<%=i %>"><%=i %></a> <%
}

// [다음]
if(endPage < pageCount) {
	%><a href="notice2.jsp?pageNum=<%=startPage+pageBlock %>">Next</a><%
}
%>
<!-- 		1 2 3 .... 10 [다음] -->
<!-- [이전]	11 12 13 .... 20	 -->
